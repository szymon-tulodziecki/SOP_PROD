...
from core.models import Internship, UserRole, InternshipStatus, EventType
...
def _stage_info(event, actor_role) -> tuple[str, str, str]:
    ...
    if event.event_type == EventType.COMMITTEE_DECISION and actor_role not in (
        UserRole.ADMIN,
        UserRole.UOPZ,
    ):
...
def _decision_history_entries(enrollment):
    ...
    for event in enrollment.process_events:
        ...
        executed_at = (
            event.executed_at.strftime("%d.%m.%Y, %H:%M") if event.executed_at else t("Brak daty")
        )
...
def lista_praktyk():
    ...
    return render_template(
        "zarzadzanie/praktyki.html",
        praktyki=praktyki,
        do_usuniecia=do_usuniecia,
        csrf_form=csrf_form,
    )
...
def przelacz_aktywnosc_praktyki(id):
    ...
    p.status = (
        ...
        if p.status == InternshipStatus.ACTIVE
    ...
    flash(
        t(
            "Praktyka {rok} ({semestr}) została {stan}.",
            rok=p.academic_year,
            semestr=p.semester_label,
            stan=stan,
        ),
        "success",
    )
...
class FormularzPrzypiszUOPZ(FlaskForm):
    supervisor_id = SelectField(
        lazy_t("Opiekun uczelniany (UOPZ)"), choices=[], validators=[Optional()]
    )
...
def lista_zgloszen():
    ...
    try:
        applications = _repo_zapisow.lista_zgloszen_strona(
            status_filter=status_filter, strona=page, supervisor_id=supervisor_id
        )
    except ValueError:
    ...
    csrf_form = FlaskForm()
    return render_template(
        "zarzadzanie/enrollments/list.html", zgloszenia=applications, csrf_form=csrf_form
    )
...
def przypisz_uopz(id):
    ...
    form.supervisor_id.choices = [("", t("--- brak ---"))] + [
        (str(u.id), f"{u.first_name} {u.last_name}") for u in uopz_list
    ]
    ...
    if form.validate_on_submit():
        if form.supervisor_id.data:
            try:
                ...
                flash(
                    t("Opiekun UOPZ przypisany, zgłoszenie przekazane do zatwierdzenia."), "success"
                )
    ...
    return render_template(
        "zarzadzanie/enrollments/przypisz_uopz.html", form=form, zapis=enrollment
    )
...
def szczegoly_zgloszenia(id):
    ...
    return render_template(
        "zarzadzanie/enrollments/szczegoly.html",
        zapis=enrollment,
        form=form,
...
def potwierdz_zapis(id):
    supervisor_id = current_user.id if current_user.role == UserRole.UOPZ else None
    try:
        _serwis_praktyk.approve_by_supervisor(
            id, actor_id=current_user.id, supervisor_id=supervisor_id
        )
        flash(
            t(
                "Zapis studenta na praktykę został potwierdzony. Zostałeś/aś przypisany/a jako opiekun."
            ),
            "success",
        )
    except IllegalTransitionError as e:
...
def usun_praktyke(id):
    ...
    if p.enrollments:
        ...
        flash(
            t(
                "Praktyka {opis} została dezaktywowana i zostanie trwale usunięta po 7 dniach. Możesz ją przywrócić.",
                opis=opis,
            ),
            "warning",
        )
        ...
        db.session.commit()
        flash(
            t("Praktyka {opis} została trwale usunięta (brak zapisanych studentów).", opis=opis),
            "success",
        )
    return redirect(url_for(_ROUTE_LISTA_PRAKTYK))
