...
def nowy_student():
    ...
    if form.validate_on_submit():
        ...
        flash(
            t(
                "Konto studenta {imie} {nazwisko} (nr alb. {album}) zostało utworzone. Student może się teraz zalogować przez Microsoft ({email}).",
                imie=student.first_name,
                nazwisko=student.last_name,
                album=student.album_number,
                email=student.email,
            ),
            "success",
        )
...
def edytuj_studenta(id):
    ...
    if request.method == "POST":
        current_app.logger.info(
            "edytuj_studenta POST: validate=%s errors=%s", form.validate(), form.errors
        )
    if form.validate_on_submit():
...
def nowy_pracownik():
    ...
    if form.validate_on_submit():
        try:
            ...
            return render_template(
                "zarzadzanie/formularz_pracownika.html", form=form, uzytkownik=None
            )

        flash(
            t(
                "Konto {imie} {nazwisko} [{rola}] utworzone. Użytkownik może się zalogować przez Microsoft ({email}).",
                imie=user.first_name,
                nazwisko=user.last_name,
                rola=user.role_label,
                email=user.email,
            ),
            "success",
        )
...
def edytuj_pracownika(id):
    ...
    if request.method == "POST":
        current_app.logger.info(
            "edytuj_pracownika POST: validate=%s errors=%s", form.validate(), form.errors
        )
    if form.validate_on_submit():
        ...
        flash(
            t(
                "Dane pracownika {imie} {nazwisko} zostały zaktualizowane.",
                imie=user.first_name,
                nazwisko=user.last_name,
            ),
            "success",
...
def import_csv():
    ...
    if form.validate_on_submit():
        ...
        if results["created"]:
            flash(
                t("Import zakończony: {liczba} kont utworzonych.", liczba=results["created"]),
                "success",
            )
...
def przelacz_aktywnosc(id):
    ...
    flash(
        t(
            "Konto {imie} {nazwisko} zostało {stan}.",
            imie=user.first_name,
            nazwisko=user.last_name,
            stan=status_label,
        ),
