...
def _check_company_uniqueness(form, exclude_id=None) -> str | None:
    ...
    if company_repository.find_active_by_name(company_name, exclude_id=exclude_id):
        return t("Firma o tej nazwie już istnieje w systemie.")
    ...
    if tax_id:
        ...
        if existing_company:
            return t(
                'Firma z NIP/KRS "{nip}" już istnieje ({nazwa}).',
                nip=tax_id,
                nazwa=existing_company.name,
            )
    return None
...
def lista_firm():
    ...
    return render_template(
        "zarzadzanie/firmy/lista.html", firmy=companies, wiersze=wiersze, csrf_form=csrf_form
    )
...
def dodaj_firme():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(t("Firma została dodana do systemu."), "success")
...
def edytuj_firme(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(t("Dane firmy zostały zaktualizowane."), "success")
        return redirect(url_for(COMPANY_LIST_ROUTE))
...
def usun_firme(id):
    ...
    if internship_count > 0:
        flash(
            t("Nie można usunąć firmy - ma {liczba} praktyk w historii.", liczba=internship_count),
            "error",
        )
        return redirect(url_for(COMPANY_LIST_ROUTE))
    ...
    db.session.commit()
    flash(t('Firma "{nazwa}" została trwale usunięta z systemu.', nazwa=company_name), "success")
    return redirect(url_for(COMPANY_LIST_ROUTE))
...
def przelacz_aktywnosc_firmy(id):
    ...
    if company.is_active:
        ...
        if active_internships > 0:
            flash(
                t(
                    "Nie można dezaktywować firmy - ma {liczba} aktywnych praktyk.",
                    liczba=active_internships,
                ),
                "error",
            )
            return redirect(url_for(COMPANY_LIST_ROUTE))
        company.is_active = False
        flash(t("Firma została dezaktywowana."), "success")
    else:
        company.is_active = True
        flash(t("Firma została aktywowana."), "success")
