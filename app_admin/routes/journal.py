...
def lista_dziennikow():
    ...
    if szukaj:
        ...
        q = q.join(Uzytkownik, ZapisPraktyki.student_id == Uzytkownik.id).filter(
            db.or_(
                Uzytkownik.first_name.ilike(wzorzec),
                Uzytkownik.last_name.ilike(wzorzec),
            )
        )

    zapisy = q.order_by(ZapisPraktyki.enrolled_at.desc()).paginate(
        page=strona, per_page=20, error_out=False
    )

    dane = []
    for z in zapisy.items:
        ostatni = db.session.query(func.max(WpisDziennika.entry_date)).filter_by(
            enrollment_id=z.id
        ).scalar()
        liczba_wpisow = db.session.query(func.count(WpisDziennika.id)).filter_by(
            enrollment_id=z.id
        ).scalar()
    ...
    return render_template('journal/lista.html',
                           dane=dane,
                           zapisy=zapisy)
...
def dziennik_zapisu(id):
    ...
    return render_template('journal/dziennik.html',
                           zapis=zapis,
                           wpisy=wpisy,
                           przerwy=przerwy,
