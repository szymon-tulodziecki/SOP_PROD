...
from core.models import (AgreementStatus, EnrollmentStatus, InternshipAgreement,
                         InternshipEnrollment, InternshipPath, UserRole)
...
def porozumienia_lista():
    ...
    porozumienia = (
        ...
        .order_by(InternshipAgreement.created_at.desc())
        .all()
    )
    wiersze = [
        {'p': p, 'badge': agreement_status_badge(p.status.value)}
        for p in porozumienia
    ]
    csrf_form = FlaskForm()
    return render_template('dziekanat/porozumienia.html',
                           grupy=grupy, porozumienia=wiersze, csrf_form=csrf_form)
...
def wyslij_porozumienie():
    ...
    zapisy = (
        db.session.query(InternshipEnrollment)
        .filter(InternshipEnrollment.id.in_(ids))
        .all()
    ) if ids else []
    ...
    flash(t('Porozumienie dla {firma} ({liczba} stud.) zostało wysłane na adres {email}.',
            firma=agreement.company_name, liczba=len(zapisy), email=recipient_email),
          'success')
    return redirect(url_for(_ROUTE_LISTA))
