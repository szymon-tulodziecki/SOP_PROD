...
def lista_ocen():
    ...
    data = GradingService.prepare_grading_list(
        supervisor_id=supervisor_id, filtr=request.args.get("filtr")
    )
    ...
    if request.method == "POST":
        try:
            grade_data = GradeFormData(
                ...
                supervisor_grade=EvaluationService.parse_grade(
                    request.form.get("supervisor_grade", "")
                ),
                workplace_grade=EvaluationService.parse_grade(
                    request.form.get("workplace_grade", "")
                ),
...
def ocen_zapis(id):
    ...
    efekty_wiersze = [
        {"efekt": o, **split_outcome_description(o.description, o.id)} for o in outcomes
    ]
    return render_template(
...
def zakoncz_zapis(id):
    ...
    flash(
        t(
            "Praktyka studenta {imie} {nazwisko} została zakończona.",
            imie=enrollment.student.first_name,
            nazwisko=enrollment.student.last_name,
        ),
        "success",
    )
    return redirect(url_for("evaluation.lista_ocen"))
    ...
    try:
        ...
        if exc.status_code:
            current_app.logger.warning(
                "tex-service returned %s for protokol %s", exc.status_code, id
            )
