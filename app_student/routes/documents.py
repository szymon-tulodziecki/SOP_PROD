...
from app_student.models import ZapisPraktyki, StatusZapisu, HarmonogramPraktyki
...
def _dok_gen(nazwa, zapis_id, typ, opis=None, dostepny=True, powod=None):
    """Dynamiczny dokument generowany przez praktyki.generuj_dokument_praktyki."""
    return {'nazwa': nazwa, 'opis': opis, 'zapis_id': str(zapis_id),
            'typ_gen': typ, 'dostepny': dostepny, 'powod': powod}
...
def moje_dokumenty():
    ...
    for zapis in zapisy:
        ...
        if sciezka == 'STANDARD':
            ...
            if firma_bez_umowy:
                docs.append(_dok_gen('Zał. 1 – Porozumienie uczelnia ↔ zakład',
                    zapis.id, 'ZALACZNIK_3', 'Dla firm bez stałej umowy z ANS'))
            docs.append(_dok_dyn('Zał. 2 – Program praktyki',
                zapis.id, 'ZAL_2', 'Z danymi studenta i firmy'))
            docs.append(_dok_gen('Zał. 2a – Indywidualny Program Praktyk',
                zapis.id, 'ZALACZNIK_4',
...
def _build_context(zapis, typ):
    ...
    if typ in ('ZAL_6',):
        from app_student.models import WpisDziennika
        ...
        ctx['wpisy'] = [{
            ...
            'efekt_nr': f"{w.learning_outcome_id:02d}" if w.learning_outcome_id else '--',
        } for w in wpisy]
    return ctx
