...
class InternshipAgreement(db.Model):
    ...
    status = db.Column(
        ...
        nullable=False, default=AgreementStatus.SENT,
    )

    created_by_id = db.Column(UUID(as_uuid=True), db.ForeignKey(_FK_USERS, ondelete='SET NULL'), nullable=True)
    ...
    signer_name     = db.Column(db.String(255), nullable=True)  # reprezentant zakładu (może się różnić od odbiorcy)
    signer_position = db.Column(db.String(255), nullable=True)
    company_notes   = db.Column(db.Text, nullable=True)

    created_by  = db.relationship('User')
    enrollments = db.relationship('AgreementEnrollment', back_populates='agreement',
...
class AgreementEnrollment(db.Model):
    ...
    agreement_id  = db.Column(UUID(as_uuid=True),
                              db.ForeignKey('internship_agreements.id', ondelete='CASCADE'),
                              primary_key=True)
    enrollment_id = db.Column(UUID(as_uuid=True),
    ...
    enrollment = db.relationship('InternshipEnrollment',
                                 backref=db.backref('agreement_links', passive_deletes=True))
