...
class JournalEntry(db.Model):
    ...
    enrollment_id = db.Column(
        UUID(as_uuid=True), db.ForeignKey(_FK_ENROLLMENTS, ondelete="CASCADE"), nullable=False
    )
    ...
    learning_outcomes = db.relationship(
        "LearningOutcome", secondary=entry_outcomes, lazy="subquery"
    )
...
class OutcomeAssessment(db.Model):
    ...
    learning_outcome_id = db.Column(
        db.Integer, db.ForeignKey(_FK_LEARNING_OUTCOMES), nullable=False
    )
    result = db.Column(
        db.Enum(
            AssessmentResult,
...
class CommitteeOutcomeEvaluation(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "enrollment_id", "learning_outcome_id", name="uq_committee_outcome_enrollment"
        ),
    )
