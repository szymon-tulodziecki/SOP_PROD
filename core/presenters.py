...
_COMMITTEE_DECISION_BADGES = {
    ...
    "PARTIALLY_APPROVED": (
        "status--in-progress",
        "Opinia częściowo pozytywna",
        "Częściowo pozytywna",
        "u-card-border-top-warning",
    ),
}
_COMMITTEE_DECISION_DEFAULT = (
    "status--odrzucona",
    "Opinia negatywna",
    "Negatywna",
    "u-card-border-top-danger",
)
...
def committee_decision_badge(decision: str | None) -> dict | None:
    if not decision:
        return None
    cls, label, label_short, border = _COMMITTEE_DECISION_BADGES.get(
        decision, _COMMITTEE_DECISION_DEFAULT
    )
...
def log_event_badge(event_type_value: str, actor_role_value: str | None) -> dict:
    ...
    return {
        "label": t(LOG_EVENT_LABELS.get(event_type_value, event_type_value)),
        "cls": "status--szkic",
    }
...
def role_badges(user) -> list[dict]:
    ...
    return [
        {"value": v, "css": ROLE_BADGE_CSS.get(v, "student")} for v in _ROLE_ORDER if v in values
    ]
...
def schedule_summary(efekty, harmonogram_dict) -> dict:
    ...
    if suma > LIMIT_DNI_MAX:
        badge_cls, total_cls, alert = (
            "status--draft u-status-danger",
            "u-day-total--high",
            "za_duzo",
        )
...
def outcome_result_badge(result_value: str) -> dict:
    label, cls, row_cls = _OUTCOME_RESULT_BADGES.get(
        result_value, (result_value, "status--draft", "")
    )
