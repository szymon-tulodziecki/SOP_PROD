...
class Uzytkownik(UserMixin, db.Model):
    ...
    tryb_studiow          = db.Column(db.String(20), nullable=True)  # 'stacjonarne'/'niestacjonarne'
...
class ZapisPraktyki(db.Model):
    ...
    @ocena_e.expression
    def ocena_e(cls):
        """SQL expression: średnia z niepustych ocen sprawdzianowych."""
        ...
        # NULLIF zapobiega dzieleniu przez zero
        return func.round(cast(total / func.nullif(count, 0), db.Numeric(5, 2)), 2)
    ...
    @ocena_k.expression
    def ocena_k(cls):
        """SQL expression: 0.4*E + 0.1*S + 0.2*U + 0.3*Z (NULL jeśli brak którejkolwiek)."""
        from sqlalchemy import case, cast, func
        e = cls.ocena_e  # używa expression powyżej
        s = cast(cls.ocena_sprawozdania, db.Float)
        u = cast(cls.ocena_uopz, db.Float)
        z = cast(cls.ocena_zopz, db.Float)
        return case(
            (
                (e.isnot(None)) & (cls.ocena_sprawozdania.isnot(None)) &
                (cls.ocena_uopz.isnot(None)) & (cls.ocena_zopz.isnot(None)),
...
class IndywidualnyProgram(db.Model):
    ...
    # Relacje
    enrollment = db.relationship('ZapisPraktyki', backref=db.backref('indywidualny_program', passive_deletes=True))
...
journal_entry_outcomes = db.Table(
    'journal_entry_outcomes',
    db.Column('journal_entry_id', UUID(as_uuid=True), db.ForeignKey('journal_entries.id', ondelete='CASCADE'), primary_key=True),
    db.Column('learning_outcome_id', db.Integer, db.ForeignKey('learning_outcomes.id'), primary_key=True),
)
...
class WpisDziennika(db.Model):
    ...
    efekty_uczenia = db.relationship('EfektUczenia', secondary=journal_entry_outcomes, lazy='subquery')
...
class UploadedDocument(db.Model):
    ...
    # Relacje
    enrollment = db.relationship('ZapisPraktyki', backref=db.backref('uploaded_documents', passive_deletes=True))
