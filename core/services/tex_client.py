...
class TexServiceError(Exception):
    ...
    def __init__(
        self, message: str, status_code: int | None = None, error_detail: str | None = None
    ):
...
def generuj_pdf(
    template: str, context: dict, filename: str | None = None, timeout: float = 60.0
) -> bytes:
...
def dyspozycja_pdf(base_name: str, last_name: str | None = None) -> str:
    ...
    ascii_fallback = (
        unicodedata.normalize("NFKD", full_name).encode("ascii", "ignore").decode("ascii").strip()
        or "dokument.pdf"
    )
