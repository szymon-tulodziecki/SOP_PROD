...
def _ext_zal2a(ctx: dict, zapis) -> None:
    ...
    ctx["schedule"] = [
        {
            "efekt_kod": (
                h.learning_outcome.code
                if h.learning_outcome
                else str(h.learning_outcome_id).zfill(2)
            ),
            ...
            "wpisy": [
                {
                    ...
                    "efekt_nr": (
                        ", ".join(f"{e.id:02d}" for e in w.learning_outcomes)
...
def _ext_zal4a(ctx: dict, zapis) -> None:
    ...
    oceny_map = {
        ...
        for e in db.session.query(CommitteeOutcomeEvaluation)
        .filter_by(enrollment_id=zapis.id)
        .all()
    }
    ...
    ctx["zapis"].update(
        {
            ...
            "supervisor": (
                {"first_name": supervisor.first_name, "last_name": supervisor.last_name}
                if supervisor
                else None
            ),
...
def _ctx_student(s) -> dict:
    return {
        ...
        "tryb_studiow": _STUDY_MODE_MAP.get(
            _g(s, "study_mode", "").lower(), _g(s, "study_mode", "")
        ),
    }
