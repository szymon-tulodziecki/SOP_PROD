...
def _render_email(title: str, paragraphs: list[str], cta_url: str = '',
                  cta_label: str = '') -> str:
...
def notify_submitted_to_supervisor(enrollment) -> None:
    ...
    try:
        ...
        html = _render_email(
            ...
            cta_url=admin_panel_url(), cta_label='Przejdź do panelu',
        )
...
def notify_student_revision(enrollment, comment: str = '') -> None:
    ...
    try:
        ...
        html = _render_email('Zgłoszenie wymaga uzupełnień', paragraphs,
                             cta_url=student_panel_url(), cta_label='Przejdź do panelu studenta')
...
def notify_student_rejected(enrollment, comment: str = '') -> None:
    ...
    try:
        ...
        html = _render_email('Zgłoszenie praktyki odrzucone', paragraphs,
                             cta_url=student_panel_url(), cta_label='Przejdź do panelu studenta')
...
def notify_agreement_recipient(agreement, token: str) -> None:
    ...
    try:
        students = ', '.join(
            ...
            for ae in agreement.enrollments if ae.enrollment and ae.enrollment.student
        )
        html = _render_email(
            ...
            cta_url=agreement_form_url(token), cta_label='Uzupełnij porozumienie',
        )
        _enqueue(agreement.recipient_email,
                 'ANS Elbląg: porozumienie w sprawie praktyk studenckich', html)
    except Exception:
