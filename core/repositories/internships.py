...
class InternshipRepository:
    ...
    def znajdz_po_id(self, internship_id: uuid.UUID) -> Optional[Internship]:
        return self.find_by_id(internship_id)
    ...
    def zapisz(self, internship: Internship) -> Internship:
        return self.save(internship)

    def usun(self, internship: Internship) -> None:
        self.delete(internship)


class EnrollmentRepository:
    """Dostęp do zapisów studentów na edycje praktyk."""

    def znajdz_po_id(self, enrollment_id: uuid.UUID) -> Optional[InternshipEnrollment]:
        return db.session.get(InternshipEnrollment, enrollment_id)
    ...
    def dla_praktyki(self, internship_id: uuid.UUID) -> list[InternshipEnrollment]:
        return (
            db.session.query(InternshipEnrollment)
    ...
    def dla_opiekuna(self, supervisor_id: uuid.UUID) -> list[InternshipEnrollment]:
        return (
            db.session.query(InternshipEnrollment)
    ...
    def pending_dla_studenta_i_praktyki(
        self, student_id: uuid.UUID, internship_id: uuid.UUID
    ) -> Optional[InternshipEnrollment]:
        """Zwraca istniejący (nie-odrzucony) zapis studenta dla danej edycji lub None."""
        return (
            db.session.query(InternshipEnrollment)
            .filter(
                InternshipEnrollment.student_id == student_id,
                InternshipEnrollment.internship_id == internship_id,
                InternshipEnrollment.status != EnrollmentStatus.REJECTED,
            )
            .first()
        )

    def student_ma_aktywny_zapis(self, student_id: uuid.UUID, internship_id: uuid.UUID) -> bool:
    ...
    def aktywny_dla_studenta(
        self, student_id: uuid.UUID, statusy: list
    ) -> Optional[InternshipEnrollment]:
    ...
    def aktywne_dla_studenta(
        self, student_id: uuid.UUID, statusy: list
    ) -> list[InternshipEnrollment]:
    ...
    def w_trakcie_strona(
        self, szukaj: str = "", supervisor_id=None, strona: int = 1, na_strone: int = 20
    ):
        """Lista zapisów IN_PROGRESS i COMPLETED z opcjonalnym filtrem UOPZ i wyszukiwaniem."""
        from sqlalchemy.orm import selectinload
        from core.models.users import Student, User

        q = (
            db.session.query(InternshipEnrollment)
            .options(selectinload(InternshipEnrollment.student))
            .filter(
                InternshipEnrollment.status.in_(
                    [
        ...
        if szukaj:
            ...
            q = q.join(User, InternshipEnrollment.student_id == User.id).filter(
                db.or_(User.first_name.ilike(wzorzec), User.last_name.ilike(wzorzec))
            )
    ...
    def aktywne_i_zakonczone(self, supervisor_id=None) -> list[InternshipEnrollment]:
        ...
        q = (
            ...
            .options(
                ...
                selectinload(InternshipEnrollment.company),
        ...
        return (
            q.join(InternshipEnrollment.student)
            .order_by(InternshipEnrollment.status.desc(), InternshipEnrollment.enrolled_at.desc())
            .all()
        )
    ...
    def lista_zgloszen_strona(
        self, status_filter: str = "", strona: int = 1, na_strone: int = 25, supervisor_id=None
    ):
        ...
        q = (
            ...
            .filter(
                db.or_(
                    ...
                    (InternshipEnrollment.status == EnrollmentStatus.AWAITING_APPROVAL)
                    & ma_komentarz_uopz,
                )
            )
        )
    ...
    def dla_uopz_strona(
        self, supervisor_id, status_filter: str = "", strona: int = 1, na_strone: int = 25
    ):
    ...
    def liczniki_dla_uopz(self, supervisor_id) -> dict:
        ...
        row = (
            db.session.query(
                ...
                func.count(
                    case((InternshipEnrollment.status == EnrollmentStatus.AWAITING_APPROVAL, 1))
                ).label("oczekujace"),
                func.count(
                    case((InternshipEnrollment.status == EnrollmentStatus.IN_PROGRESS, 1))
                ).label("zatwierdzone"),
            )
            .filter(InternshipEnrollment.supervisor_id == supervisor_id)
            .one()
    ...
    def liczniki_nav(self, supervisor_id=None) -> dict:
        ...
        from core.models.internships import FinalGrades
        ...
        has_supervisor = exists().where(
            and_(
                FinalGrades.enrollment_id == InternshipEnrollment.id,
        ...
        ungraded_b = and_(
            InternshipEnrollment.status.in_(
                [EnrollmentStatus.COMPLETED, EnrollmentStatus.IN_PROGRESS]
            ),
    ...
    def liczba_aktywnych_dla_studentow(self, student_ids: list, aktywne_statusy: list) -> dict:
        """Zwraca {student_id: count} dla listy studentów — eliminuje N+1."""
        if not student_ids:
            return {}
        rows = (
            db.session.query(
                InternshipEnrollment.student_id, func.count(InternshipEnrollment.id).label("cnt")
            )
            .filter(
                InternshipEnrollment.student_id.in_(student_ids),
    ...
    def statystyki_pulpit(self, supervisor_id=None) -> dict:
        ...
        row = q.with_entities(
            ...
            func.count(case((InternshipEnrollment.status == EnrollmentStatus.COMPLETED, 1))).label(
                "zakonczone"
            ),
        ).one()
    ...
    def ostatnie(self, supervisor_id=None, limit: int = 8) -> list[InternshipEnrollment]:
        ...
        q = db.session.query(InternshipEnrollment).options(
            selectinload(InternshipEnrollment.student),
            selectinload(InternshipEnrollment.company),
        )
        if supervisor_id is not None:
    ...
    def harmonogram_dla_zapisu(self, enrollment_id: uuid.UUID) -> list[InternshipSchedule]:
        return db.session.query(InternshipSchedule).filter_by(enrollment_id=enrollment_id).all()
    ...
    def znajdz_odrzucony(self, student_id, internship_id) -> Optional[InternshipEnrollment]:
        return (
            db.session.query(InternshipEnrollment)
            .filter_by(
                student_id=student_id, internship_id=internship_id, status=EnrollmentStatus.REJECTED
            )
            .first()
        )
    ...
    def ostatnie_zdarzenie(
        self, enrollment_id, event_type=None, decision=None
    ) -> Optional[ProcessEvent]:
        q = db.session.query(ProcessEvent).filter_by(enrollment_id=enrollment_id)
