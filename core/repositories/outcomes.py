...
class OutcomeRepository:
    ...
    def wszystkie(self) -> list[LearningOutcome]:
        return (
            db.session.execute(db.select(LearningOutcome).order_by(LearningOutcome.id))
            .scalars()
            .all()
        )
    ...
    def po_ids(self, ids: list[int]) -> list[LearningOutcome]:
        ...
        if not ids:
            return []
        return (
            db.session.execute(db.select(LearningOutcome).filter(LearningOutcome.id.in_(ids)))
            .scalars()
            .all()
        )
